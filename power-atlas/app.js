...
function gradeText(grade) {
  return {
    A: 'Primary evidence',
    B: 'Strong evidence',
    C: 'Supported secondary evidence',
    D: 'Inference',
    E: 'Unverified proposal'
  }[grade] || 'Unrated';
}
...
function buildGraph() {
  ...
  for (const node of state.data.nodes) {
    elements.push({ data: { ...node, label: node.name } });
  }
  ...
  state.cy = cytoscape({
    ...
    style: [
      ...
      {
        selector: '.search-hit',
        style: { 'border-width': 5, 'width': 58, 'height': 58 }
      }
    ],
    layout: {
      name: 'cose',
...
function isAliveAt(node, year) {
  ...
  const bornOk = node.born == null || year >= node.born;
  const diedOk = node.died == null || year <= node.died;
  return bornOk && diedOk;
}

function edgeActive(edge, year) {
  const startOk = edge.start == null || year >= edge.start;
  const endOk = edge.end == null || year <= edge.end;
...
function updateTimeline() {
  ...
  state.cy.batch(() => {
    state.cy.nodes().forEach(ele => {
      ele.toggleClass('inactive-time', !isAliveAt(ele.data(), state.year));
    });
    state.cy.edges().forEach(ele => {
      ele.toggleClass('inactive-time', !edgeActive(ele.data(), state.year));
    });
  });
  applyFilters();
}
...
function applyFilters() {
  ...
  state.cy.batch(() => {
    state.cy.nodes().forEach(ele => {
      ele.toggleClass('filtered-out', !state.activeTypes.has(ele.data('type')));
    });
...
function showNode(node) {
  ...
  const years = node.type === 'person' && node.born
    ? `${node.born}${node.died ? `–${node.died}` : '–'}`
    : '';
  ...
  body.innerHTML = `
    ...
    <div class="relation-list">
      ${connected.map(edge => {
        ...
        return `<button class="relation-row" data-edge="${esc(edge.id)}">
          <span>${esc(other?.name || otherId)}</span>
          <small>${esc(edge.label)} · Grade ${esc(edge.grade)}</small>
        </button>`;
...
function showEdge(edge) {
  ...
  const sourceLinks = (edge.sources || []).map(s => `
    <a class="source-link" href="${esc(s.url)}" target="_blank" rel="noopener noreferrer">
      <span>${esc(s.title)}</span><span aria-hidden="true">↗</span>
    </a>`).join('');
  ...
  body.innerHTML = `
    ...
    <div class="meta-grid">
      <div><span>Period</span><strong>${esc(range)}</strong></div>
      <div><span>Evidence</span><strong>Grade ${esc(edge.grade)} · ${esc(gradeText(edge.grade))}</strong></div>
    </div>
    <p>${esc(edge.note || '')}</p>
